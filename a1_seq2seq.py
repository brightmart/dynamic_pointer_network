...
#for pointer network
def rnn_decoder_with_attention_ptr_network(decoder_inputs, initial_state, cell, loop_function,attention_states,decoder_input,scope=None):#3D Tensor [batch_size x attn_length x attn_size]
    ...
    Args:
        ...
        decoder_input:[batch_size, self.decoder_sent_length]
    ...
    with tf.variable_scope(scope or "rnn_decoder"):
        ...
        outputs = []
        prev = None
        decoder_inputs_tensor=tf.stack(decoder_inputs,axis=1) #[batch_size,decoder_sequence_length, input_size]
         #################################################
        for i, inp in enumerate(decoder_inputs):#循环解码部分的输入。如sentence_length个[batch_size x input_size]
            ...
            if loop_function is not None and prev is not None:#测试的时候：如果loop_function不为空且前一个词的值不为空，那么使用前一个的值作为RNN的输入
               with tf.variable_scope("loop_function", reuse=True):
                    ...
                    print("0.prev_symbol:",prev_symbol)
                    inp = tf.gather(decoder_inputs_tensor, prev_symbol)
                    print("0.inp:", inp)
            #if it is not training(inference), we use the value associate with the position pointed by the pointer
            if i > 0:
            ...
            #attention_final=tf.multiply(attention_states_original,p_attention) #[batch_size x attn_length x attn_size]. attention_states_original: it is represent input X.
            #context_vector=tf.reduce_sum(attention_final,axis=1)     #[batch_size x attn_size].  context_vector: the weighted sum of encode input representing the important information at current timestamp.
            ############################################################################################################################################################
            #inp:[batch_size x input_size].it is decoder input;  attention_final:[batch_size x attn_size]
            #print("inp:", inp, ";state:", state); #inp=(?, 200);state:(?, 200)
            output, state =cell(inp, state) # cell(inp, state,context_vector)
            #output=p_attention #tf.argmax(p_attention,axis=1) # Tensor(shape=(?,), dtype=int64). [batch_size]
            #print(";output(single):", output) #(?, 5)
            outputs.append(p_attention) # 将输出添加到结果列表中
